...
GO
/****** Object:  StoredProcedure [dbo].[AGW0009]    Script Date: 21-05-2018 16:45:12 ******/
SET ANSI_NULLS ON
GO
...
-----------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------
FECHA DE MODIFICACIÓN: 28/06/2018
USUARIO DE MODIFICACIÓN: VICTOR CORONADO
MOTIVO DE MODIFICACIÓN: SE MAPEA CON TABLA RL_TDA_ST PARA OBTENER SECTORES TRANSVERSALES,
						SE QUITA PARAMETRIZACIÓN POR TABLA RL_TAW_ST
-----------------------------------------------------------------------------------------------
VISADO POR (QA): 
FECHA APROBACIÓN QA: 
COMENTARIOS QA: 
...
where 
       ...
       AND ((not exists(SELECT 1 FROM RL_TDA_ST RL WHERE RL.TDA_ID  = SGH.TDA_ID AND RL.ELIMINADO = 0) AND SGH.SEC_ID = @SEC_ID) 
             or (exists(SELECT 1 FROM RL_TDA_ST RL WHERE RL.TDA_ID  = SGH.TDA_ID AND RL.ELIMINADO = 0)))
